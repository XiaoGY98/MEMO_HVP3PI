...
\begin{equation}
    \sigma_{\varepsilon} = \sqrt{\frac{\varepsilon(1-\varepsilon)}{N_{\text{total}}}},
\end{equation}
where $N_{\text{total}}$ is the total number of events in the corresponding sample.
...
The yields in data for each selection category ($b_{\text{LO}}$, $b_{\text{NLO LA}}$, $b_{\text{NLO SA}}$) are obtained from the fits described in Sections~\ref{sec:nlo_la} and~\ref{sec:nlo_sa} for the NLO LA and SA categories, while the LO yield in data has been previously studied in Fig.~\ref{fig:signal_yields}. These observed yields are related to the unknown generator-level yields $x_i$ (where $i$ denotes LO, NLO LA, or NLO SA) through the efficiency matrix:
\[
    b_j = \sum_i \mathcal{A}_{ji} x_i,
\]
where $\mathcal{A}_{ji}$ is the probability for a truth-type $i$ event to pass selection $j$. This forms a $3 \times 3$ linear system that can be solved for $x_i$:
\[
    x = \mathcal{A}^{-1} b.
\]

Solving this system using the data yields gives the unfolded generator-level event counts in data, as summarized in Table~\ref{tab:yields_comparison}. For comparison, the generator-level fractions from the PHOKHARA MC sample, computed directly from the truth-level classification (including the same $M(3\pi)$ requirement), are also presented alongside the data fractions.

\begin{table}[htbp]
    \centering
    \caption{Generator-level event yields and fractions in data after unfolding with the efficiency matrix, compared with the truth-level fractions from PHOKHARA MC.}
    \vspace{4mm}
    \begin{tabular}{lccc}
        \hline
        \multirow{2}{*}{Category} & \multicolumn{2}{c}{Data} & \multirow{2}{*}{MC Fraction (\%)} \\
        \cline{2-3}
        & Yield & Fraction (\%) & \\
        \hline
        ...
        \hline
    \end{tabular}
    \label{tab:yields_comparison}
\end{table}

Table~\ref{tab:nlo_fraction_unfolded} presents the comparison between data and MC for the three categories. The uncertainties on the unfolded data fractions are propagated from the uncertainties on the fitted yields through the matrix inversion.

\begin{table}[htbp]
    \centering
    \caption{Comparison of generator-level event fractions between data and PHOKHARA MC after unfolding with the efficiency matrix. The correction factor is defined as the ratio of data to MC fractions and will be applied in the unfolding procedure.}
    \vspace{4mm}
    \begin{tabular}{lccc}
        \hline
        Category & MC Fraction (\%) & Data Fraction (\%) & Correction factors (Data/MC) \\
        \hline
        LO       & 76.88 & $77.88 \pm 1.45$ & $1.013 \pm 0.019$ \\
        NLO LA   & 6.54  & $9.85 \pm 1.57$  & $1.506 \pm 0.240$ \\
        NLO SA   & 16.58 & $12.26 \pm 0.92$ & $0.740 \pm 0.056$ \\
        \hline
    \end{tabular}
    \label{tab:nlo_fraction_unfolded}
\end{table}
