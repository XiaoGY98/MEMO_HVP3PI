...
It is emphasized that both the $\chi^2$ and the uncertainty estimates are computed strictly within the three intervals and corresponding bin widths defined in Section~\ref{DataYields}. Using the MC sample as a stand-in for data, a scan is performed over a grid of $\lambda_L$, $\lambda_U$, and $n_{\text{iter}}$ values, and the combination that best satisfies these criteria is selected for the final unfolding of the data. The optimized parameters obtained from this procedure for the three representative mass regions are summarized in Table~\ref{tab:optimized_parameters}.

Since reconstructed events near the boundaries may have truth-level counterparts originating from outside the region of interest, we extend the boundaries for each mass region during the training of the unfolding procedure. This extension is applied solely to preserve the completeness of information within the defined region and does not affect the $\chi^2$ or uncertainty calculations, which remain confined to the intervals in Section~\ref{DataYields}. Events falling outside the region of interest are not considered further. The unfolding results obtained with these optimized parameters are shown in Fig.~\ref{fig:mc_unfold_results}.

\begin{table}[t]
    \begin{center}
        \caption{Optimized unfolding parameters for the three representative mass regions. For each region, the parameters $\lambda_L$, $\lambda_U$, $\lambda_M$, and number of iterations $n_{\text{iter}}$ were selected based on the $\chi^2$ minimization procedure described in the text. The resulting $\chi^2/\text{ndf}$ values indicate good agreement between the unfolded and true MC distributions.}
        \vspace{4mm}
        \renewcommand{\arraystretch}{1.3} % 增加行高
        \begin{tabular}{c | c | c | c | c | c}
            \hline \hline
            Region             & $\lambda_L$ & $\lambda_U$ & $\lambda_M$ & $n_{\text{iter}}$ & $\chi^2/\text{ndf}$ \\
            \hline
            $\omega(782)$      & $0.01$      & $0.01$      & $1.0$       & $3$               & $29.32/40$          \\[2pt] % 添加额外垂直间距
            \hline
            $\phi(1020)$       & $0.1$       & $0.1$       & $1.0$       & $1$               & $21.91/30$          \\[2pt]
            \hline
            $\omega'/\omega''$ & $0.1$       & $0.1$       & $1.0$       & $3$               & $12.72/76$          \\[2pt]
            \hline \hline
        \end{tabular}
...
\begin{figure}[htbp]
    \centering
    \subfloat[$\omega(782)$ region with IDS unfolding]{
        \includegraphics[width=0.32\textwidth]{fig/mc_unfold_omega.eps}
        \label{fig:unfold_omega}
    }
    \hfill
    \subfloat[$\phi(1020)$ region with IDS unfolding]{
        \includegraphics[width=0.32\textwidth]{fig/mc_unfold_phi.eps}
        \label{fig:unfold_phi}
    }
    \hfill
    \subfloat[$\omega^{\prime}/\omega^{\prime\prime}$ region with IDS unfolding]{
        \includegraphics[width=0.32\textwidth]{fig/mc_unfold_high.eps}
        \label{fig:unfold_high}
    }
    \caption{Unfolded results for three representative mass regions using the IDS method with optimized parameters. Each panel shows the true MC distribution (black), the measured MC distribution (red), and the unfolded result (blue points) with its statistical uncertainties. The MC sample has been normalized to match the data statistics shown in Fig.~\ref{fig:signal_yields}, ensuring that the uncertainties properly reflect the data statistical power.}
    \label{fig:mc_unfold_results}
\end{figure}

To validate the unfolding procedure, we compare the relative statistical uncertainty of the unfolded spectrum with that of the test MC sample. The relative uncertainty of the test MC spectrum, $\delta_{\text{test}}$, is computed by summing in quadrature the Poisson errors of each bin and dividing by the total yield:
\begin{equation}
