...
\caption{Unfolded results for three representative mass regions using the IDS method with optimized parameters. Each panel shows the true MC distribution (blue), the smeared MC distribution (black), and the unfolded result (red points) with its statistical uncertainties. The iteration number and regularization parameters ($\lambda_L$, $\lambda_U$) are indicated in each panel. The MC sample has been normalized to match the data statistics shown in Fig.~\ref{fig:signal_yields}, ensuring that the uncertainties properly reflect the data statistical power.}
\label{fig:unfold_results}
\end{figure}