...
Candidate events are selected by requiring exactly two good charged tracks, accompanied by at least one reconstructed $\pi^0$ and a minimum of three good photon showers. This ensures the presence of at least one $\pi^0$ and one isolated $\gamma_{\mathrm{ISR}}$. A four-constraint (4C) kinematic fit is then applied to the event, and the candidate with the smallest $\chi_{\mathrm{4C}}^2$ value is chosen for further analysis.
