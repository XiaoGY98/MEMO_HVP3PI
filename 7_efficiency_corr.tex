...
\subsection{The $\chi^2_{4C}$ distribution and correcting the signal yields}
\label{corr_signal_yields}
The discrepancy in the $\chi^2_{4C}$ distribution between data and MC is corrected following the method established in a dedicated study using $e^+e^- \to K^+K^-\pi^+\pi^-$ events~\cite{BESIII_chisq4C_correction_2025}. That analysis, performed under BOSS version 7.1.2, investigated the differences in helix parameters between data and MC simulation across various data-taking periods at the center-of-mass energy of 3.773~GeV. For the signal process $e^+e^- \to \pi^+\pi^-\pi^0\gamma_{\text{ISR}}$, we apply helix parameter corrections to the charged tracks. Figures~\ref{fig:chisq4c_before} and~\ref{fig:chisq4c_after} show the comparison of $\chi^2_{4C}$ distributions between data and MC before and after the correction, respectively. As illustrated, a slight improvement in the agreement between data and MC is observed in the low $\chi^2_{4C}$ region, where the majority of events are populated, after applying the helix parameter corrections. With these corrections applied to the signal MC, we subsequently impose a selection requirement of $\chi^2_{4C} < 50$ and refit the signal yields in each $M_{3\pi}$ interval.

\begin{figure}[htbp]
    \centering
    \subfloat[Before correction]{
        \includegraphics[width=0.48\textwidth]{fig/LO_kmfit4C_comparison_comparison.eps}
        \label{fig:chisq4c_before}
    }
    \hfill
    \subfloat[After correction]{
        \includegraphics[width=0.48\textwidth]{fig/LO_kmfit4C_corr_comparison_comparison.eps}
        \label{fig:chisq4c_after}
    }
    \caption{Comparison of $\chi^2_{4C}$ distributions between data and MC for the $e^+e^- \to \pi^+\pi^-\pi^0\gamma_{\text{ISR}}$ process (a) before and (b) after applying helix parameter corrections.}
    \label{fig:chisq4c_comparison}
\end{figure}
...
\clearpage
\subsection{Correcting the Response Matrix for Unfolding}
\label{sec:correction_method}
To construct the migration matrix for unfolding, we start from the signal MC samples that have been corrected for helix parameters and satisfy the $\chi^2_{4C} < 50$ selection requirement. For each truth bin $j$ (defined in $M_{3\pi}$), we have three basic quantities from the original MC:
\begin{itemize}
    \item $N_{\text{total},j}$: the total number of generated events in that truth bin (before any corrections);
    \item $N_{\text{pass},j}$: the number of events that are successfully reconstructed and pass all selection criteria, which are distributed among various reconstructed bins $i$;
    \item $N_{\text{miss},j} = N_{\text{total},j} - N_{\text{pass},j}$: the number of events that fail reconstruction or selection.
\end{itemize}

The migration matrix element $A_{ij}$ is intended to represent the number of events that truly belong to bin $j$ and are reconstructed in bin $i$. However, the MC simulation does not perfectly describe data; discrepancies arise from the NLO ISR photon production rate and from the efficiencies of tracking, PID, $\pi^0$ reconstruction, and ISR photon detection. These must be corrected before the matrix can be used for unfolding.

The correction is applied in two steps. First, the NLO photon fraction discrepancy is corrected at the generator level. For each truth bin $j$, the corrected total number of generated events becomes
\[
N_{\text{total},j}^{\text{corr}} = c_{\text{NLO}} \cdot N_{\text{total},j},
\]
where $c_{\text{NLO}}$ is the bin-dependent NLO correction factor.

Second, each reconstructed event is assigned a weight that accounts for the efficiency corrections:
\[
w_p = c_{\text{trk}}(\pi^+) \cdot c_{\text{pid}}(\pi^+) \cdot c_{\text{trk}}(\pi^-) \cdot c_{\text{pid}}(\pi^-) \cdot c_{\pi^0} \cdot c_{\gamma_{\text{ISR}}},
\]
where each factor depends on the momentum of the corresponding particle. This weight adjusts the MC reconstruction yield to match the data. The sum of these weights over all reconstructed events originating from truth bin $j$ gives the corrected number of weighted passing events:
\[
N_{\text{pass},j}^{\text{corr}} = \sum_{\text{reconstructed events from truth bin }j} w_p.
\]

To preserve the total number of truth events after correction, the missing events must also be weighted appropriately. Let $w_m$ be the weight applied to each missing event from truth bin $j$. The condition
\[
N_{\text{total},j}^{\text{corr}} = N_{\text{pass},j}^{\text{corr}} + w_m \cdot N_{\text{miss},j}
\]
yields
\[
w_m = \frac{N_{\text{total},j}^{\text{corr}} - N_{\text{pass},j}^{\text{corr}}}{N_{\text{miss},j}}.
\]

Finally, the corrected migration matrix $A_{ij}^{\text{corr}}$ is filled by iterating over all reconstructed events: for an event from truth bin $j$ that falls into reconstructed bin $i$, we add its weight $w_p$ to $A_{ij}^{\text{corr}}$. The missing events, weighted by $w_m$, are not placed into any reconstructed bin, thereby correctly accounting for the overall efficiency loss. The normalized response matrix $P_{ij}^{\text{corr}}$ is then obtained by applying Eq.~\eqref{eq:response_matrix_norm} to $A_{ij}^{\text{corr}}$.

Using the corrected migration matrix $A_{ij}^{\text{corr}}$, the unfolding of the $M_{3\pi}$ distribution is performed on data from Sec.~\ref{corr_signal_yields}. To avoid edge effects, the mass range for each resonance region is appropriately extended during the unfolding procedure, then truncated back to the nominal interval defined in Tab.~\ref{tab:optimized_parameters} for the final results. Figure~\ref{fig:unfolded_data_results} presents the unfolded $M_{3\pi}$ distributions for data in the three regions.

\begin{figure}[htbp]
    \centering
    \subfloat[$\omega(782)$ region]{
        \includegraphics[width=0.32\textwidth]{fig/unfolded_omega.eps}
        \label{fig:unfolded_omega}
    }
    \hfill
    \subfloat[$\phi(1020)$ region]{
        \includegraphics[width=0.32\textwidth]{fig/unfolded_phi.eps}
        \label{fig:unfolded_phi}
    }
    \hfill
    \subfloat[$\omega^{\prime}/\omega^{\prime\prime}$ region]{
        \includegraphics[width=0.32\textwidth]{fig/unfolded_high.eps}
        \label{fig:unfolded_high}
    }
    \caption{Unfolded $M_{3\pi}$ distributions for data after applying the corrected response matrix. The results are shown only within the nominal mass ranges defined in Tab.~\ref{tab:optimized_parameters}. }
    \label{fig:unfolded_data_results}
\end{figure}