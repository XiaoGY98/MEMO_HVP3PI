...
Following the selection procedures described above for both LA and SA categories, we obtain samples of events containing an identified NLO ISR photon. For each category, we define the NLO photon fraction as:

\[
    R_{\text{NLO}} = \frac{N_{e^+e^- \to \pi^+\pi^-\pi^0\gamma_{\text{ISR}}\gamma_{\text{NLO}}}}{N_{e^+e^- \to \pi^+\pi^-\pi^0\gamma_{\text{ISR}}}}
\]

where the denominator corresponds to events passing the nominal signal selection (4C fit \(\chi^2 < 50\) with a single ISR photon), and the numerator is the number of events from this sample that also satisfy the respective LA or SA NLO photon selection criteria. The fractions are computed separately for data and for the PHOKHARA signal MC sample.

Table~\ref{tab:nlo_fraction} presents the measured NLO photon fractions for both categories. The statistical uncertainties are calculated assuming Poisson statistics. To quantify the level of agreement between data and simulation, we compute the ratio \(R_{\text{NLO}}^{\text{Data}} / R_{\text{NLO}}^{\text{MC}}\) for each category.

\begin{table}[htbp]
    \centering
    \begin{tabular}{lcc}
        \hline
        Category         & \(R_{\text{NLO}}^{\text{Data}}\) & \(R_{\text{NLO}}^{\text{MC}}\) \\
        \hline
        Large Angle (LA) & \(0.XX \pm 0.XX\)                & \(0.XX \pm 0.XX\)              \\
        Small Angle (SA) & \(0.XX \pm 0.XX\)                & \(0.XX \pm 0.XX\)              \\
        \hline
    \end{tabular}
    \caption{Comparison of NLO photon fractions between data and PHOKHARA MC for large angle and small angle categories.}
    \label{tab:nlo_fraction}
\end{table}

The observed ratios indicate whether PHOKHARA overestimates or underestimates the production of additional ISR photons in our channel. A significant deviation from unity, particularly in the small angle category where the BaBar effect was observed, would suggest a similar discrepancy in our simulation. This comparison serves as an important cross-check of the generator's accuracy for our signal process and will inform any necessary adjustments to systematic uncertainties associated with ISR modeling.
