...
MC studies show that the main backgrounds include $e^{+}e^{-} \to \pi^{+}\pi^{-}\pi^{0}\pi^{0}$, $e^{+}e^{-} \to \pi^{+}\pi^{-}\pi^{0}\pi^{0}\gamma_{\text{ISR}}$, and various QED processes.

To verify that these processes constitute the primary backgrounds, a rough estimate is obtained by multiplying the integrated luminosity $\mathcal{L}$ by the hadronic cross section $\sigma_{\text{hadron}}$. The luminosity values for each data-taking period are listed in Table~\ref{tab:datalumi}. The background yield is estimated as a function of the $\pi^{+}\pi^{-}\pi^{0}$ invariant mass ($m$) using the effective luminosity spectrum, which is related to the mass spectrum via:
\begin{equation}
...
\end{align}
where $\alpha$ is the fine-structure constant, $m_e = 0.5110034 \times
...
After applying the optimal $\chi^2_{\mathrm{KF}}$ selection, the invariant mass
...
\caption{Invariant mass spectra $M_{3\pi}$ for different data rounds and mass regions, $N_{\pi^0}=1$ is applied for the $\omega^{\prime}/\omega^{\prime\prime}$ mass region.}
...
\clearpage
\subsection{Estimate of $\ee\to\pip\pim\piz\piz$ backgrounds} 

It is assumed that the total yield of $e^{+}e^{-} \to \pi^{+}\pi^{-}\pi^{0}\pi^{0}$ events produced in the data is a fixed quantity. This process can contribute background to the $e^{+}e^{-} \to \pi^{+}\pi^{-}\pi^{0}\gamma_{\text{ISR}}$ process when a photon from one of the $\pi^{0}$ decays is misidentified as the initial-state radiation photon ($\gamma_{\text{ISR}}$). The combined selection proceeds as follows.

First, the full $e^{+}e^{-} \to \pi^{+}\pi^{-}\pi^{0}\pi^{0}$ selection is applied:

\begin{itemize}
    \item \textbf{$\pi^{+}\pi^{-}\pi^{0}\pi^{0}$ Candidate Selection:}
    \begin{itemize}
        \item Exactly two good charged tracks.
        \item $\text{nPi0} \geq 2$ and $\text{nGoodShowers} \geq 4$ (ensuring at least two $\pi^{0}$s).
        \item Application of a total 4-momentum constraint to the initial $e^{+}e^{-}$ collision (4C kinematic fit).
        \item Looping over all combinations of $\pi^0$ candidates, the combination with the minimum $\chi_{\mathrm{4C}}^2$ is chosen.
        \item A requirement of $\chi_{\mathrm{KF}}^{2} < 10$ on the kinematic fit.
    \end{itemize}
\end{itemize}

Subsequently, within the same event, a loose selection for $e^{+}e^{-} \to \gamma_{\mathrm{ISR}}\pi^{+}\pi^{-}\pi^{0}$ is applied to reconstruct the $3\pi$ invariant mass spectrum:

\begin{itemize}
    \item \textbf{Loose $\gamma_{\mathrm{ISR}}\pi^{+}\pi^{-}\pi^{0}$ Candidate Selection:}
    \begin{itemize}
        \item The same two good charged tracks.
        \item $\text{nPi0} \geq 1$ and $\text{nGoodShowers} \geq 3$ (ensuring at least one $\pi^{0}$ and one photon assigned as $\gamma_{\mathrm{ISR}}$).
        \item Application of the same total 4-momentum constraint to the initial $e^{+}e^{-}$ collision (4C kinematic fit).
        \item Looping over all combinations of $\pi^0$ candidates and EMC showers, the combination with the minimum $\chi_{\mathrm{4C}}^2$ is chosen.
        \item No additional kinematic fit $\chi^2$ requirement is applied at this stage.
    \end{itemize}
\end{itemize}

To estimate the background from $4\pi$ events to the $3\pi$ signal region, two efficiencies are defined using MC based on the selections above:

\begin{itemize}
    \item $\epsilon_{\text{sig}}$: the efficiency for $e^{+}e^{-} \to \pi^{+}\pi^{-}\pi^{0}\pi^{0}$ events to pass the nominal $4\pi$ selection (thereby allowing the yield of this process in data to be measured), and then to additionally satisfy the loose $\gamma_{\mathrm{ISR}}\pi^{+}\pi^{-}\pi^{0}$ selection for reconstructing the $3\pi$ invariant mass spectrum.
    \item $\epsilon_{\text{bkg}}$: the efficiency for the same $4\pi$ events to pass the $e^{+}e^{-} \to \pi^{+}\pi^{-}\pi^{0}\gamma_{\text{ISR}}$ selection described in Sec.~\ref{sec:selections}, i.e., to be misreconstructed as background in the $3\pi$ mass region.
\end{itemize}

The observed distribution of $e^{+}e^{-} \to \pi^{+}\pi^{-}\pi^{0}\pi^{0}$ candidates in data (selected under the nominal $4\pi$ criteria) can be converted into an estimate of its background contribution in the signal region using the efficiency ratio $R = \epsilon_{\text{bkg}} / \epsilon_{\text{sig}}$:

\begin{equation}
    N_{\text{bkg}} = R \times N_{4\pi}^{\text{obs}} = \frac{\epsilon_{\text{bkg}}}{\epsilon_{\text{sig}}} \times N_{4\pi}^{\text{obs}},
\end{equation}

where $N_{4\pi}^{\text{obs}}$ is the number of observed $e^{+}e^{-} \to \pi^{+}\pi^{-}\pi^{0}\pi^{0}$ events selected under the nominal $4\pi$ criteria, and $N_{\text{bkg}}$ is the estimated number of background events from this process in the $\pi^{+}\pi^{-}\pi^{0}$ invariant mass region.
...
this selected sample is negligible. The resulting
...
For each small $M_{3\pi}$ interval, the signal yield is extracted through an extended unbinned maximum-likelihood fit to the $U_{\text{miss}}$ distribution, where $U_{\text{miss}} = E_{\text{miss}} - |\vec{p}_{\text{miss}}|$ is computed from the missing four-momentum after removing the $\pi^+$, $\pi^-$, $\pi^0$, and $\gamma_{\text{ISR}}$ from the initial $e^+e^-$ system. These quantities are calculated using the measured momenta without any kinematic fit update. The fit model incorporates probability density functions (PDFs) representing the signal and all background components.
...
\begin{itemize}
    \item \textbf{Signal PDF:} Histogram shape from signal MC, convolved with a Gaussian for resolution correction. The $\chi^{2}_{\mathrm{KF}}$ used in the $\chi^{2}_{\mathrm{KF}} < 50$ selection criterion has been corrected for data-MC differences in helix parameters as described in Section~\ref{sec:corr_kmfit}.
